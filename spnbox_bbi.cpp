#include <iostream>
#include <cstdint>
#include <cstring>
#include <ctime>
#include <fstream>
#include <x86intrin.h>

#define blocknum 128 // 3072 byte
#define l 4 
#define ln 4 
#define R 16 

#define xtime(a) (((a) << (1)) ^ ((((a) >> (7)) & 1) * (0x1b)))
#define ltime(a) (((a) << (1)) ^ ((((a) >> (31)) & 1) * (0x8d)))

uint8_t invSB[256]={};

using namespace std;

void invSboxLayer(uint32_t& state){
    uint32_t tmp=0;
    for(int j=l-1;j>=0;j--){
        tmp|=(invSB[(state>>8*j)&0xFF]<<8*j);
    }
    state=tmp; 
}

void addroundkeyLayer(uint32_t& state, uint32_t roundkey){state^=roundkey;}

void invmixColumnLayer(uint32_t &state){

    uint8_t tmp[4] = {};
    uint8_t st[4] = {};

    for(int j=0;j<4;j++){
        tmp[j] = state&0xFF; 
        state=state>>8;
    }
    
    st[0] = xtime(xtime(xtime(tmp[0]^tmp[1]^tmp[2]^tmp[3]))) ^ xtime(xtime(tmp[0]^tmp[2])) ^ xtime(tmp[0]^tmp[1]) ^ tmp[1]^tmp[2]^tmp[3];
    st[1] = xtime(xtime(xtime(tmp[0]^tmp[1]^tmp[2]^tmp[3]))) ^ xtime(xtime(tmp[1]^tmp[3])) ^ xtime(tmp[1]^tmp[2]) ^ tmp[0]^tmp[2]^tmp[3];
    st[2] = xtime(xtime(xtime(tmp[0]^tmp[1]^tmp[2]^tmp[3]))) ^ xtime(xtime(tmp[0]^tmp[2])) ^ xtime(tmp[2]^tmp[3]) ^ tmp[0]^tmp[1]^tmp[3];
    st[3] = xtime(xtime(xtime(tmp[0]^tmp[1]^tmp[2]^tmp[3]))) ^ xtime(xtime(tmp[1]^tmp[3])) ^ xtime(tmp[0]^tmp[3]) ^ tmp[0]^tmp[1]^tmp[2];
    
    for(int j=3;j>=0;--j){
        state = state<<8; 
        state ^= st[j];
    } 
}

void invNonlinearLayer(uint32_t *state){

    uint32_t key[0x10]={0x46b9dd2b,0x0ba88d13,0x233b3feb,0x743eeb24,0x3fcd52ea,0x62b81b82,0xb50c2764,0x6ed5762f,
                   0xd75dc4dd,0xd8c0f200,0xcb05019d,0x67b592f6,0xfc821c49,0x479ab486,0x40292eac,0xb3b7c4be};

    for(int j=0;j<ln;j++){
        for(int i=15; i>0; i--){
            addroundkeyLayer(state[j],key[i]);
            invmixColumnLayer(state[j]);
            invSboxLayer(state[j]);
        }
        addroundkeyLayer(state[j], key[0]);
    }
}

void invLinearLayer(uint32_t *state)  //theta
{
    uint32_t temp[4]={};

    temp[0] = state[0]; temp[1] = state[1]; temp[2] = state[2]; temp[3] = state[3];

    state[0] = temp[0] ^ ltime(temp[1]^temp[3]) ^ ltime(ltime(temp[2]^temp[3])) ^ ltime(ltime(ltime(ltime(temp[0]^temp[2])))) ^ ltime(ltime(ltime(ltime(ltime(ltime(temp[0]^temp[1]^temp[2]^temp[3]))))));
    state[1] = temp[1] ^ ltime(temp[0]^temp[2]) ^ ltime(ltime(temp[0]^temp[3])) ^ ltime(ltime(ltime(ltime(temp[1]^temp[3])))) ^ ltime(ltime(ltime(ltime(ltime(ltime(temp[0]^temp[1]^temp[2]^temp[3])))))); 
    state[2] = temp[2] ^ ltime(temp[1]^temp[3]) ^ ltime(ltime(temp[0]^temp[1])) ^ ltime(ltime(ltime(ltime(temp[0]^temp[2])))) ^ ltime(ltime(ltime(ltime(ltime(ltime(temp[0]^temp[1]^temp[2]^temp[3]))))));
    state[3] = temp[3] ^ ltime(temp[0]^temp[2]) ^ ltime(ltime(temp[1]^temp[2])) ^ ltime(ltime(ltime(ltime(temp[1]^temp[3])))) ^ ltime(ltime(ltime(ltime(ltime(ltime(temp[0]^temp[1]^temp[2]^temp[3]))))));
}

void affineLayer(uint32_t *state, int round) //sigma_r
{
    for(int j=0;j<ln;j++){state[j]^=((round-1)*ln+j+1);}
}

void dec(uint32_t *state){
    for(int i=R;i>0;i--){
        affineLayer(state, i);
        invLinearLayer(state);
        invNonlinearLayer(state);
    } 
}

void generatemessage(uint32_t input[blocknum][ln]){
	for(int i=0; i<blocknum; i++){
        for(int j=0;j<ln;j++){
            input[i][j]=rand();
        }
	}
}

int main(){
    uint32_t input[blocknum][ln]={};
    uint32_t state[blocknum][ln]={};

    int iter=100000;

    srand(time(0));
    uint64_t start_time,end_time,cyc=0;

    generatemessage(input);

    for(int count=0;count<iter;count ++){
        for(int j=0;j<blocknum;j++){
            for(int i=0;i<ln;i++){state[j][i]=input[j][i];} 
        }
        
        for(int j=0;j<blocknum;j++){
            start_time=_rdtsc();
            dec(state[j]);
            end_time=_rdtsc();
            cyc+=end_time-start_time;
        }
    }

    uint64_t aver=cyc/iter;

    cout << "average cost " << aver << " CPU cycles for " << iter << " test" << endl;
	uint64_t cpb = aver/(4*ln*blocknum);
    cout << "average CPB for decryption is " << cpb << endl;

    return 0;
}